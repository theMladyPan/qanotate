...
class ImageNavigator(QMainWindow):
    ...
    def keyPressEvent(self, event):
        ...
        # all numbers on numeric keypad
            ...
            # annotate the image with the number pressed
            current_image = self.image_list[self.current_index]
            self.image_status[current_image] = event.text()
            print(f"Marked {current_image} as {event.text()}")
            self.next_image(event)
